...
create cached table T_TAG ( TAG_ID_C varchar(36) not null, TAG_IDUSER_C varchar(36) not null, TAG_PUBLIC_C BOOLEAN NOT NULL, TAG_NAME_C varchar(36) not null, TAG_COLOR_C varchar(7) default '#3a87ad' not null, TAG_CREATEDATE_D datetime, TAG_DELETEDATE_D datetime, primary key (TAG_ID_C) );
...
create memory table T_AUDIOBOOK ( AUD_ID_C VARCHAR(36) not null, AUD_EXTID_C VARCHAR(50) not null, AUD_NAME_C VARCHAR(1000) NOT NULL, AUD_AUTHORS_C VARCHAR(250) NOT NULL, AUD_TMBURL_C VARCHAR(250) NOT NULL, AUD_DESC_C VARCHAR(10000) NOT NULL, AUD_URL_C VARCHAR(250) NOT NULL, PRIMARY KEY (AUD_ID_C));
create memory table T_PODCAST ( POD_ID_C VARCHAR(36), POD_EXTID_C VARCHAR(50), POD_NAME_C VARCHAR(1000) NOT NULL, POD_TMBURL_C VARCHAR(250) NOT NULL, POD_DURN_C FLOAT NOT NULL, POD_URL_C VARCHAR(1000) NOT NULL, PRIMARY KEY (POD_ID_C) );
create memory table T_USER_AUDIOBOOK ( UAB_ID_C VARCHAR(36), UAB_UID_C VARCHAR(36) NOT NULL, UAB_ABID_C VARCHAR(36) NOT NULL, PRIMARY KEY (UAB_ID_C) );
create memory table T_USER_PODCAST ( UP_ID_C VARCHAR(36), UP_UID_C VARCHAR(36) NOT NULL, UP_PODID_C VARCHAR(36) NOT NULL, PRIMARY KEY (UP_ID_C) );
...
alter table T_USER_AUDIOBOOK add constraint FK_UAB_USER_C foreign key (UAB_UID_C) references T_USER (USE_ID_C) on delete restrict on update restrict;
alter table T_USER_AUDIOBOOK add constraint FK_UAB_AUDIOBOOK_C foreign key (UAB_ABID_C) references T_AUDIOBOOK (AUD_ID_C) on delete restrict on update restrict;
alter table T_USER_PODCAST add constraint FK_USER_PODCAST_C foreign key (UP_UID_C) references T_USER (USE_ID_C) on delete restrict on update restrict;
alter table T_USER_PODCAST add constraint FK_USER_PODCAST_C foreign key (UP_PODID_C) references T_PODCAST (POD_ID_C) on delete restrict on update restrict;
