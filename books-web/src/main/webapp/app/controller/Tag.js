...
App.controller('Tag', function($scope, $state, Restangular) {
  $scope.tag = { name: '', color: '#3a87ad' , isPublic : false };
  ...
  $scope.addTag = function() {
    Restangular.one('tag').put($scope.tag).then(function(data) {
      $scope.tags.push({ id: data.id, name: $scope.tag.name, color: $scope.tag.color , isPublic: $scope.tag.isPublic });
      $scope.tag = { name: '', color: '#3a87ad' , isPublic:false };
    });
  };
   ...
   * Make a tag public.
   */
  $scope.makeTagPublic = function(tag) {
    console.log('making public', tag);
  };
